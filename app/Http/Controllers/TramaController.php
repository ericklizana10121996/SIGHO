...
class TramaController extends Controller
{
    ...
    protected $rutas           = array('raiz'=>'tramag',
            'nueva' => 'trama.nueva', 
            'edit'   => 'trama.edit', 
    ...
    public function listarD(Request $request){
        ...
        $resultado        = Movimiento::join('person as paciente','paciente.id','=','movimiento.persona_id')
                            ...
                            // ->whereIn('movimiento.numero',[11162])
    ...
    function generarAte($lista,$periodo,$lote,$plan,$fecha,$ruc,$ipress,$v_iafa){
        ...
        $resultado        = Movimiento::join('person as paciente','paciente.id','=','movimiento.persona_id')
                            ...
                            ->whereIn('movimiento.id',$lista);
        $resultado        = $resultado->orderBy('movimiento.serie', 'ASC')->orderBy('movimiento.numero', 'ASC')
                            ->select('movimiento.id as movid','cie.codigo as cie10','movimiento.subtotal','movimiento.igv','movimiento.total','movimiento.codigocobertura as cobertura',DB::raw('plan.nombre as plan'),'historia.carnet','movimiento.tipomovimiento_id','movimiento.serie','movimiento.numero',DB::raw('movimiento.fecha'),DB::raw('TIME(movimiento.created_at) as hora2'),'historia.tipopaciente','movimiento.copago',DB::raw('historia.numero as historia'),'movimiento.tarjeta','movimiento.tipotarjeta','movimiento.situacion as situacion2','historia.numero as historia','paciente.dni',DB::raw('responsable.nombres as responsable'),DB::raw('movimiento.numero as numero2'),'empresa.bussinesname','empresa.direccion','empresa.ruc','movimiento.comentario','movimiento.fechaingreso','movimiento.fechaalta','cie.descripcion as cie','movimiento.soat');
        $lista            = $resultado->get();
        $contador = 0;
        foreach ($lista as $key => $value){
        //if ($value->comentario != '' && $value->cie10 != '') {
            ...
            foreach ($laboratorio as $key => $valor){
                ...
                $tarif =TarifarioSusalud::where('codigoSusalud','=',$valor->tarifariosusalud_id)->select('codigoSusalud','idRubro')->first();
                if(!is_null($tarif)){
            ...
            //////////////////////////////////
            $v_cobertura = (is_null($value->cobertura) || $value->cobertura == "" || $value->cobertura == "0")?"4100":$value->cobertura;
            // $v_cobertura = $v_cobertura.' ';
            ...
            $value->dni != '' || !is_null($value->dni) ? $vdni = $value->dni : $vdni = '00000000';
    ...
    function generarSer($lista,$periodo,$lote,$plan,$fecha,$ruc,$ipress,$v_iafa){
        ...
        foreach ($lista as $key => $value){
            ...
            //             // dd($value->servicio2);
            //             // $cadena_aux = $cadena_aux.' ** '.$value->servicio2;
            //          }else{
