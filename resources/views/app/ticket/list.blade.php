	...
	<table id="example1" class="table table-bordered table-striped table-condensed table-hover">
		...
		<tbody>
			...
			<tr>
	            ...
	            <td>{{ $value->numero }}</td>
	 			<?php $aux = explode(' ', $value->paciente);?>
	            <td>{{$aux[1]. ' '. $aux[2]. ' '. (count($aux)>3?$aux[3]:'') }}</td>
				...
				@elseif(($user->usertype_id==1 || $user->usertype_id==7 || $user->usertype_id==2) && $value->situacion!='U' && $value->total>0)
					<td align="center">{!! Form::button('<div class="glyphicon glyphicon-pencil"></div>', array('onclick' => 'modal (\''.URL::route($ruta["editarresponsable"], array($value->id, 'listar'=>'SI')).'\', \''.$titulo_modificar.'\', this);', 'class' => 'btn btn-xs btn-success', 'title' => 'Editar')) !!}</td>
				@else
					<td align="center"> - </td>
				@endif
