<?php
if(is_null($reciboblancoingreso)){
    $person_id=1;
    $persona=null;
    $fecha=date('Y-m-d');
    $historia_id=null;
    ...
    $person_id=$reciboblancoingreso->persona_id;
    $persona=$reciboblancoingreso->persona->apellidopaterno.' '.$reciboblancoingreso->persona->apellidomaterno.' '.$reciboblancoingreso->persona->nombres;
    $fecha=$reciboblancoingreso->fecha;
    $historia_id=null;
    $historia=null;
    $tipopaciente=null;
    $doctor_id=$reciboblancoingreso->doctor_id;
    $doctor=$reciboblancoingreso->doctor->apellidopaterno.' '.$reciboblancoingreso->doctor->apellidomaterno.' '.$reciboblancoingreso->doctor->nombres;
    $concepto=$reciboblancoingreso->comentario;
    $total=$reciboblancoingreso->total;
}
?>
<div id="divMensajeError{!! $entidad !!}"></div>
{!! Form::model($reciboblancoingreso, $formData) !!}	
    		...
    		if(respuesta === 'ERROR'){
    		  ...
    		  //alert(respuesta);
    			...
    			if (resp === 'OK') {
                    ...
                    window.open('/juanpablo/reciboblancoingreso/pdfRecibo?id='+dat[0].id,'_blank')
    			} else if(resp === 'ERROR') {
    				alert(dat[0].msg);
    			} else {
    				mostrarErrores(respuesta, idformulario, entidad);
