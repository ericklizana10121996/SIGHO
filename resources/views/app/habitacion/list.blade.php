...
<table id="example1" class="table table-bordered table-striped table-condensed table-hover">
	...
	<tbody>
		...
		<tr>
			<td>{{ $contador }}</td>
            <td align="center">{{ $value->tipohabitacion->nombre }}</td>
            <td align="center">{{ $value->piso->nombre }}</td>
            <td align="center">{{ $value->nombre }}</td>
            <td align="center">{{ $value->sexo }}</td>
            <td align="center">{{ $value->situacion=='D'?'DISPONIBLE':($value->situacion=='O'?'OCUPADO':'SUCIA') }}</td>
